...
Run the stop command even if a prior step fails. The test database uses the distinct `tally-tracker-test` Compose project and `postgres-test` service on `127.0.0.1:5433` by default. Set `TALLY_TEST_DB_PORT` on each command if that port is unavailable. Its data directory is tmpfs and does not reuse the development volume.
