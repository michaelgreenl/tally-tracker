...
- **🛑 HITL Gate:** User reviews the understanding document.
    - _If_ Confirmed: The confirmed requirements understanding becomes an input for the High-Level Planner. Move to step 0.2.
    - _If_ Refined: User provides corrections/answers. Clarifier produces updated understanding. Repeat until confirmed.
...
- **🛑 HITL Gate (first attempt only):** User reviews the plan direction.
    - _If_ Approved: Save draft to `<initiative-title>/plans/high-level-plans/high-level-NNNN/high-level-plan.md`. Move to Context Gathering (step 0.3).
    - _If_ Rejected: User provides feedback. Planner generates a revised plan in `high-level-{N+1}/`. Repeat until approved.
...
- **Routing Logic:**
    - _If_ Approved: Copy final plan to `<initiative-title>/plans/high-level-plan.md`. Move to Low-Level Planning (step 0.5).
    - _If_ Rejected: Reviewer writes `high-level-revisions.md` inside `high-level-NNNN/`. Plan loops back to HL Planner (step 0.2) with the context and revision feedback as additional inputs. The planner's revised plan routes directly back here (skipping user HITL and context gathering). Repeat until approved.
...
- **Routing Logic:**
    - _If_ Approved: Save final draft to `<initiative-title>/plans/low-level-plan.md`. Move to Phase 1.
    - _If_ Rejected: Reviewer writes `low-level-revisions.md` inside `low-level-NNNN/`. Planner generates new plan in `low-level-{N+1}/`. Repeat until approved.
...
- **Routing Logic:**
    - _If_ Rejected: Reviewer writes `revisions.md` in `tests-0001/`. Writer creates new attempt log in `tests-0002/log.md` describing test locations and addressing revisions.
    - _If_ Approved by Agent: Pauses workflow and moves to HITL Validation. Reviewers verify test files exist in correct codebase locations.
...
- **Routing Logic:**
    - _If_ User Rejects: User provides text feedback. Writer loops back and rewrites tests in codebase based on feedback.
    - _If_ User Approves: Orchestrator writes `test-manifest.md` finalizing the record of test locations and commands. Move to Phase 2.
...
- **Routing Logic:**
    - _If_ Rejected (Tests Fail / Logic Flaw): Reviewer writes `revisions.md` in `implementation-0001/`. Implementer tries again in `implementation-0002/`.
    - _If_ Approved (Tests Pass): Write `final-report.md`. Initiative complete.
    - _If_ Critical Unforeseen Bug Discovered: Move to Phase 3 (Issues).
...
- **🛑 HITL Gate:** User reviews the understanding.
    - _If_ Confirmed: Move to step 3.2 (Context Gathering).
    - _If_ Refined: User provides corrections. Clarifier re-produces. Repeat until confirmed.
...
- **Routing Logic:**
    - _If_ Rejected: Writes `revisions.md` in `attempt-0001/`. Planner and Implementer try again in `attempt-0002/`.
    - _If_ Approved: Move issue directory from `active/` to `fixed/`. Update `all-issues.md`.
