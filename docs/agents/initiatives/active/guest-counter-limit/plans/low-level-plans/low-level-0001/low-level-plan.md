# Low-Level Plan: guest-counter-limit
...
**Change 3:** Add imports for the newly mocked modules and fixtures. After the existing import on line 34:
...
**Change 4:** Add `FORBIDDEN` to the status code import on line 1:
...
| After `PUT /counters/update` block | Add new `describe('POST /counters — limit enforcement')` block with 8 test cases |
...
All existing tests should still pass (they use PREMIUM tier, which skips the guard). The 8 new tests should all pass.
...
**Change 3:** Add the `showUpgradeModal` ref. Insert after the existing `counterToUpdate` ref (after line 41):
...
**Change 4:** Add the `handleAddCounter` function. Insert before the `</script>` tag (after the `closeCounterForm` function, after line 58):
...
**Change 5:** Replace the "Add counter" button click handler. On line 102:
...
**Change 6:** Add the `ion-modal` component. Insert it **after** the closing `</div>` of `content-wrapper` (after line 107) and **before** `</ion-content>` (line 108):
...
**Change 7:** Add modal styling. Append to the existing `<style>` block (after line 130, before `</style>`):
...
Expected: All existing tests pass. All 8 new limit enforcement tests pass.
...
Expected output should show the `POST /counters — limit enforcement` describe block with all 8 tests passing.
...
| 5    | `app/server/src/tests/integration/specs/counters.spec.ts` | Modify (mocks + 8 tests)             | M2        |
| 6    | `app/client/src/views/UpgradeView.vue`                    | **Create**                           | M3        |
| 7    | `app/client/src/router/index.ts`                          | Modify (import + route)              | M3        |
| 8    | `app/client/src/views/HomeView.vue`                       | Modify (modal + gate)                | M3        |
