# High-Level Plan: Counter Limit for Non-Premium Accounts (v3)

> Revision of high-level-0002. Key changes from v2: (1) test mock infrastructure is now fully specified, (2) `getUserById` null return is handled in the controller guard.
...
- `app/server/src/tests/integration/specs/counters.spec.ts` — add test cases for limit enforcement. **Note:** This file requires two mock additions: (1) a new `vi.mock('../../../db/repositories/user.repository', ...)` block for `getUserById`, and (2) `countAccessibleByUser: vi.fn()` added to the existing `counter.repository` mock object.
...
2. If `user` is `null` → return `404` with `{ success: false, message: 'User not found' }`. _(This is a defensive guard — in practice, JWT validation ensures the user exists, but the DB is the source of truth.)_
3. If `user.tier !== 'PREMIUM'` → query `counterRepository.countAccessibleByUser(userId)`
4. If count >= `MAX_FREE_COUNTERS` → return `403` with `{ success: false, message: 'Counter limit reached. Upgrade to Premium for unlimited counters.' }`
...
- POST `/counters` returns `404` with `{ success: false, message: 'User not found' }` when `getUserById` returns `null` (user deleted between JWT issuance and request)
...
- `app/server/src/tests/integration/specs/counters.spec.ts` — edge-case tests (premium user, exactly-at-limit, over-limit, shared counters pushing over limit, null user)
...
Before writing new test cases, the following mock setup changes are required in `counters.spec.ts`:

1. **Add `user.repository` mock:** Add `vi.mock('../../../db/repositories/user.repository', () => ({ getUserById: vi.fn() }))` at the top level alongside the existing `counter.repository` and `idempotency.repository` mocks. Even though existing PREMIUM-tier tests never call `getUserById` (premium users skip the guard), the mock must be present to prevent import resolution errors when the controller imports `userRepository`.

2. **Add `countAccessibleByUser` to the `counter.repository` mock:** The existing `vi.mock('../../../db/repositories/counter.repository', ...)` block must include the new function:

    ```ts
    vi.mock('../../../db/repositories/counter.repository', () => ({
        // ... existing functions (post, getAllByUser, getByIdOrShare, etc.) ...
        countAccessibleByUser: vi.fn(),
    }));
    ```

3. **BASIC-tier auth mock for limit tests:** The top-level `vi.mock('../../../middleware/auth.middleware', ...)` hardcodes `tier: 'PREMIUM'`. For BASIC-tier limit enforcement tests, add a new `describe('POST /counters — limit enforcement', ...)` block that overrides the auth middleware mock via `vi.mocked(jwt).mockImplementation(...)` to set `tier: 'BASIC'` for that block only. Within these tests, use `buildUser({ tier: 'BASIC' })` from `user.fixture.ts` as the `getUserById` mock return value. This approach keeps existing PREMIUM-tier tests untouched.
...
| User not found (null)              | —     | —                 | —     | 404         |

### Success Criteria

- Server returns 404 when `getUserById` returns `null` for the authenticated user
...
| `app/server/src/tests/integration/specs/counters.spec.ts` | Modified (add tests + mock updates)    |
...
| Area | v1 (high-level-0001) | v2 (high-level-0002) | v3 (this plan) |
| --- | --- | --- | --- |
| Server count query | `prisma.counter.count({ where: { userId } })` — owned only | `prisma.counter.count({ where: { OR: [owned, accepted-share] } })` — all accessible | Unchanged from v2 |
| Client count source | Noted as "ideally owned only", suggested `ownedCounters` computed | `counterStore.counters.length` is correct as-is (includes shared) | Unchanged from v2 |
| Repository function | `countByUser` (owned only) | `countAccessibleByUser` (owned + shared, mirrors `getAllByUser` filter) | Unchanged from v2 |
| Test scenarios | Did not cover shared-counter-at-limit cases | Explicit test matrix for shared counter combinations | Added "User not found (null)" scenario to test matrix |
| Join endpoint gap | Not mentioned | Documented as known gap (join not gated by limit) | Unchanged from v2 |
| `getUserById` null handling | Not addressed | Guard pseudocode did not null-check `getUserById` result | **Fixed:** Guard step 2 returns 404 if user is null |
| Test mock infrastructure | Not addressed | Unspecified — implementer had to discover mock gaps | **Fixed:** "Test Infrastructure Requirements" subsection specifies all three mock changes (user.repository mock, countAccessibleByUser in counter.repository mock, BASIC-tier auth mock strategy) |
| Milestone 2 test file note | Not addressed | No note about mock additions needed | **Fixed:** Files Touched entry for `counters.spec.ts` notes the two required mock additions |
