...
* **Validated Fix Test** — Located at **`<initiative>/issues/active/<issue-title>/issue.{test,spec,cy}.{ts,tsx,js,jsx}`**. The immutable test file approved by the Test Reviewer and the user. This test MUST pass when your fix is complete.
...
* **Validated Fix Test** — Same as above (unchanged).
...
* **Updated Fix Test** — Located at **`<initiative>/issues/active/<issue-title>/issue.{test,spec,cy}.{ts,tsx,js,jsx}`** (newly written by the re-invoked Fix Test Writer, re-approved through HITL).
* **Issue Description** — Same as above.
...
**`<initiative>/issues/active/<issue-title>/attempt-NNNN/log.md`**
...
Identical to the Initiative Implementer, you operate from three source documents:
...
* **`issue.{test,spec,cy}.{ts,tsx,js,jsx}`** — Your **correctness oracle**. Tells you whether your fix is correct.
...
1. Run the fix-specific test: **`<issue-title>/issue.{test,spec,cy}.{ts,tsx,js,jsx}`**.
2. Run the initiative-level test suite: **`<initiative>/tests/<initiative-title>.{test,spec,cy}.{ts,tsx,js,jsx}`** (if it exists).
3. BOTH must pass.
...
4. If after **3 consecutive fix cycles** you cannot make both test suites pass simultaneously, stop and document the conflict in your log.
