...
- **Test Manifest** — Located at **`<initiative>/tests/test-manifest.md`**. Describes where the finalized, user-approved test files live in the codebase (app/client or app/server) and how to run them.
- **Validated Test Suite** — The actual executable test file(s) referenced in the test-manifest.md, located in **`app/client/` or `app/server/`**. These are the finalized, immutable test files that have been approved by the Test Reviewer and the user. Every test in these files MUST pass when your implementation is complete.
...
Write your implementation log to: **`<initiative>/implementation/implementation-NNNN/log.md`**
...
You operate from exactly three sources. Each has a distinct role:
...
- **`test-manifest.md` + actual test files in codebase** — Your **correctness oracle**. The manifest tells you where tests live and how to run them. The actual test files in `app/client/` or `app/server/` tell you whether your implementation is right or wrong. Every test must pass. If a test fails, your implementation is wrong—not the test.

Do not rely on any information source outside these sources and the project's actual source files you are modifying. Do not reference conversation history, prior discussions, or assumed knowledge.
...
1. Read the test-manifest.md to identify the test file location(s) and the command(s) to run them.
2. Run the full validated test suite using the command(s) from the manifest (e.g., `npm run test:client` or `npm run test:server`).
3. If all tests pass → write your log and declare implementation complete.
4. If any tests fail:
    ...
    - Re-run the **entire** test suite (not just the previously failing test) using the manifest commands.
    - Repeat until all tests pass, or until you have attempted **3 consecutive fix cycles** without resolution.
5. If after 3 fix cycles tests still fail:
...
- **Step [N]: [Title]** _(matching the plan's step number and title)_
...
### Issues Encountered _(include only if any)_
