...
**`<initiative>/tests/<initiative-title>-tests-NNNN/<initiative-title>.{test,spec,cy}.{ts,tsx,js,jsx}`**

Where:
* `<initiative-title>` matches the initiative directory name.
* `NNNN` is the current attempt number, zero-padded to 4 digits (e.g., `0001`, `0002`).
* The file extension MUST match the project's established test file convention as documented in **`<initiative>/plans/context.md`** (e.g., `.test.ts`, `.spec.ts`, `.cy.ts`). If the context document does not specify a convention, default to `.test.ts`.

* First attempt → **`<initiative-title>-tests-0001/<initiative-title>.test.ts`** (or project-appropriate extension).
* After revision → increment (e.g., **`<initiative-title>-tests-0002/<initiative-title>.test.ts`**).
* Create the numbered directory if it does not already exist.

When approved by **both** the Test Reviewer and the user (HITL gate), the orchestrator will copy the test file to **`<initiative>/tests/<initiative-title>.{test,spec,cy}.{ts,tsx,js,jsx}`** as the finalized version.
